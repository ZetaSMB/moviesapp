...
class MovieSearchViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        movieSearchViewModel = MovieSearchViewModel(query: searchBar.rx.text.orEmpty.asDriver(),
                                                    repository: TMDbRepository.shared)
        ...
        movieSearchViewModel.isFetching.drive(activityIndicatorView.rx.isAnimating)
            .disposed(by: disposeBag)
        
        movieSearchViewModel.info.drive(onNext: {[unowned self] (info) in
...
extension MovieSearchViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configure(with: movieSearchViewModel.viewModelItemForMovie(at: indexPath.row))
        return cell
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let vm = movieSearchViewModel.viewModelDetailForMovie(at: indexPath.row), let vcDetail = MovieDetailViewController.createMovieDetailController(detailViewModel:vm) {
