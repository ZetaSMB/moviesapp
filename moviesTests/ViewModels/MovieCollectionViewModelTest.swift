...
//        XCTAssertEqual(moviesOutput.events.count, 1)
        //above lines are equals to this
        XCTAssertEqual(moviesOutput.events, [Recorded.next(0, mockedRepository.allFetchedMovies)])
        XCTAssertEqual(sut.hasError, false)
        XCTAssertEqual(sut.numberOfMovies, mockedRepository.allFetchedMovies.count)
        
        let mvCollectionItem = sut.viewModelItemForMovie(at: 0)
        let movie = mockedRepository.allFetchedMovies[0]
        XCTAssertEqual(mvCollectionItem?.title, movie.title)
        XCTAssertEqual(mvCollectionItem?.overview, movie.overview)
        
        //
        let mvMovieDetail = sut.viewModelDetailForMovie(at: 0)
        XCTAssertEqual(mvMovieDetail?.title, movie.title)
        XCTAssertEqual(mvMovieDetail?.overview, movie.overview)
        
    }
    
    func testFetchWithError() throws {
        let firstFechedList = mockedRepository.allFetchedMovies
        //force error response on 2nd fech
        mockedRepository.forceErrorResponse = true
        let moviesOutput = scheduler.createObserver([Movie].self)
        sut.outputs.movies
            .drive(moviesOutput)
            .disposed(by: disposeBag)
        ...
        XCTAssertEqual(moviesOutput.events, [Recorded.next(0, firstFechedList), .next(10, [])])
        XCTAssertEqual(sut.hasError, true)
        XCTAssertNotNil(try sut.error.toBlocking().first())
    }
}
